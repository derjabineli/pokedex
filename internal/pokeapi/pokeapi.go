...
func CatchPokemon(pokemon string) (Pokemon, error){
	result := Pokemon{}

	res, err := http.Get("https://pokeapi.co/api/v2/pokemon/" + pokemon)
	if err != nil {
		return result, fmt.Errorf("couldn't find pokemon. error: %v", err)
	}
	...
	if res.StatusCode > 299 {
		return result, fmt.Errorf("response failed with status code: %d and body: %s", res.StatusCode, body)
	}
	if err != nil {
		fmt.Printf("Location Explore Failed. Error: %v \n", err)
	}
	...
	err = json.Unmarshal(body, &result)
	if err != nil {
		return result, fmt.Errorf("couldn't unmarshal Pokemon. error: %v", err)
	}

	chance := rand.Intn(result.BaseExperience)

	if chance > result.BaseExperience / 2 {
		return result, nil
	} else {
		return Pokemon{}, fmt.Errorf("couldn't catch pokemon")
	}
}